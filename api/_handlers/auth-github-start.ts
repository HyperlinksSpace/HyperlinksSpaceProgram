...
function getRedirectUri(request: AnyRequest): string {
  const explicitRaw = process.env.GITHUB_OAUTH_REDIRECT_URI?.trim();
  if (explicitRaw) {
    ...
    appWarn("[auth-github-start]", "invalid_env", { key: "GITHUB_OAUTH_REDIRECT_URI" });
  }
...
async function handler(request: AnyRequest, res?: NodeRes): Promise<Response | void> {
  ...
  if (!clientId) {
    const body = { ok: false, error: "github_client_id_not_configured" };
  ...
  try {
    ...
    appError("[auth-github-start]", "createLoginAttempt_failed", undefined, err);
