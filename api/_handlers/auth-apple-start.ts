...
function getRedirectUri(request: AnyRequest): string {
  const explicitRaw = process.env.APPLE_OAUTH_REDIRECT_URI?.trim();
  if (explicitRaw) {
    ...
    appWarn("[auth-apple-start]", "invalid_env", { key: "APPLE_OAUTH_REDIRECT_URI" });
  }
...
async function handler(request: AnyRequest, res?: NodeRes): Promise<Response | void> {
  ...
  if (!clientId) {
    const body = { ok: false, error: "apple_client_id_not_configured" };
  ...
  try {
    ...
    appError("[auth-apple-start]", "createLoginAttempt_failed", undefined, err);
    const body = { ok: false, error: "login_attempt_persist_failed" };
    if (res) return sendJsonViaRes(res, body, 500, request);
    return sendJson(body, 500, request);
  }

  const authUrl = buildAppleAuthorizeUrl({
    clientId,
    redirectUri,
    state,
    nonce,
