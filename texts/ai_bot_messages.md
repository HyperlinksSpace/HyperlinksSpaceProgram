...
- Receives every request with: `input`, `telegram_username`, `thread_id`, `type` (`'bot'` | `'app'`), and optionally `telegram_update_id` (bot only).
...
- Resolves `telegram_username`, `thread_id`, `update_id` from `ctx`, and passes them into the AI call (including `telegram_update_id`).
- If AI returns **skipped** (claim insert failed), returns without calling AI again and without sending any reply or draft.
- Before **each** draft send and before the **final** reply: calls `getMaxTelegramUpdateIdForThread(telegram_username, thread_id, 'bot')`. If `max !== our update_id`, aborts (does not send). No message writes in the bot; only this read for mixing.
- Sends drafts and final reply as today; does not call `insertMessage` itself.

**TMA**

- Calls the same AI layer with `telegram_username`, `thread_id`, `type: 'app'`. No `telegram_update_id`. Same persistence (user + assistant) and same history loading. No mixing logic unless we add a TMA-specific mechanism later (e.g. client request id + uniqueness).

**Data flow (bot)**

1. User sends a message → webhook → bot handler.
2. Bot: resolve `telegram_username`, `thread_id`, `update_id`; call AI with `input`, `telegram_username`, `thread_id`, `type: 'bot'`, `telegram_update_id`.
...
- **telegram_username:** `normalizeUsername(ctx.from?.username)` (same as grammy upsert). If empty, we can skip persistence or still reply (plan: skip DB only when username missing).
...
- At the start of the AI flow (after we have `text`, `telegram_username`, `thread_id`), call:
  `insertMessage({ telegram_username, thread_id, type: 'bot', role: 'user', content: text, telegram_update_id })`.
...
**Where:** `responder.ts`, right after we have `text` and before we set up streaming/cancellation. Requires `telegram_username` and `update_id`; user must exist in `users` (grammy already upserts before calling the handler).
...
- Before each **draft** send and before the **final reply**, call:
  `getMaxTelegramUpdateIdForThread(telegram_username, thread_id, 'bot')`.
...
- After we send the final reply with `ctx.reply(result.output_text, replyOptions)` (and only when we actually send, not when we aborted or errored), call:
  `insertMessage({ telegram_username, thread_id, type: 'bot', role: 'assistant', content: result.output_text })` (no `telegram_update_id`).
...
- Load history: `getThreadHistory({ telegram_username, thread_id, type: 'bot', limit })`.
...
| Step | What | Files |
|------|------|--------|
| 1 | Resolve and pass `telegram_username`, `thread_id`, `update_id` in responder | `responder.ts` |
...
- **No username:** If `telegram_username` is empty (no `ctx.from.username`), we can skip all DB calls and keep current behavior (reply without persisting), or refuse to reply; plan suggests skip persistence only.
